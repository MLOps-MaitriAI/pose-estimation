...
loaded_model = joblib.load('./models/random_forest_model.sav')  # Update the model filename to match your Random Forest model
loaded_scaler = joblib.load('./models/scaler.sav')  # Ensure the scaler file path is correct
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if landmarks:
            ...
            if prediction[0] in ['tree', 'goddess']:  # Adjust based on your categories
    ...
    st.info("Go to Sidebar and do accordingly")
    ...
    if use_webcam:
        ...
        while cap.isOpened():
            ...
            if landmarks:
                ...
                # Predict using the loaded Random Forest model
                prediction = loaded_model.predict(features_scaled)
                cv2.putText(landmarked_image, f'Predicted Category: {prediction[0]}', 
